...
export function publish(editor: Editor, editorEl: HTMLElement, stateEl: HTMLElement, markdown: string = editor.action(getMarkdown()) || '') {
	...
	if (new TextEncoder().encode(markdown).length >= 10_000_000) {
		showState(stateEl, 'note is too large')
		return
	}

	fetch(`api/publish`, {
		method: 'POST', body: JSON.stringify({ note: markdown })
